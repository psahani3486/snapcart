...
function DeliveryBoyDashboard({earning}:{earning:number}) {
  const [chartData, setChartData] = useState<{name:string;earnings:number;deliveries:number}[]>([])
...
return ()=>socket.off("update-deliveryBoy-location")
  ...
  useEffect(() => {
    fetchCurrentOrder()
    fetchAssignments()
    // fetch historical earnings for chart
    ;(async ()=>{
      try{
        ...
        // fallback to single-day chart if needed
        setChartData([{name:new Date().toLocaleDateString(undefined,{weekday:'short'}),earnings:earning||0,deliveries:(earning||0)/40}])
      }
    })()
  }, [userData])
...
if(!activeOrder && assignments.length===0){
  ...
  // If server-side prop 'earning' is still provided, keep showing it below.
  ...
  return (
    <div className='flex items-center justify-center min-h-screen bg-linear-to-br from-white to-green-50 p-6'>
      ...
      <div className='bg-white border rounded-xl shadow-xl p-6'>
        <h2 className='font-medium text-green-700 mb-2'>Performance (7 days)</h2>
