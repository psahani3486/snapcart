...
const getOrder=async ()=>{
  try {
    ...
    // API now returns { order, assignment }
    ...
    if(o.assignedDeliveryBoy && o.assignedDeliveryBoy.location){
      setDeliveryBoyLocation({
        latitude:o.assignedDeliveryBoy.location.coordinates[1],
        longitude:o.assignedDeliveryBoy.location.coordinates[0]
      })
    }
  } catch (error) {
    console.log(error)
...
return ()=>socket.off("update-deliveryBoy-location")
 ...
 setMessages((prev)=>[...prev!,message])
    ...
    socket.on("order-status-update",({orderId:oid,status})=>{
      if(oid===orderId){
...
<p className='text-sm text-gray-600'>order#{order?._id?.toString().slice(-6)} <span className='text-green-700 font-semibold'>{order?.status}</span></p>
           ...
           <div className='px-4 mt-6 space-y-4'>
              {/* Simple timeline showing order progress */}
              <div className='flex items-center gap-3 mb-2'>
                <div className={`px-3 py-2 rounded-lg ${order?.status!=='pending' ? 'bg-green-100' : 'bg-gray-100'}`}>
                  Placed
                </div>
                <div className={`px-3 py-2 rounded-lg ${order?.status==='out of delivery' || order?.status==='delivered' ? 'bg-green-100' : 'bg-gray-100'}`}>
                  Picked
                </div>
                <div className={`px-3 py-2 rounded-lg ${order?.status==='delivered' ? 'bg-green-100' : 'bg-gray-100'}`}>
                  Delivered
                </div>
              </div>
 ...
 <div className='bg-white rounded-3xl shadow-lg border p-4 h-[430px] flex flex-col'>
   ...
   {/* Pickup / Assignment details */}
   {assignment && (
     <div className='mb-4 p-3 rounded-lg bg-blue-50 border border-blue-100'>
       <div className='flex items-center justify-between'>
         <div>
           <div className='text-sm text-gray-600'>Pickup assigned to</div>
           <div className='font-semibold'>{assignment.assignedTo?.name || 'Assigned'}</div>
           <div className='text-xs text-gray-500'>📞 {assignment.assignedTo?.mobile || '—'}</div>
         </div>
         <div className='text-right text-sm text-gray-600'>
           <div>Pickup time</div>
           <div className='font-medium'>{assignment.pickupTime ? new Date(assignment.pickupTime).toLocaleString() : assignment.acceptedAt ? new Date(assignment.acceptedAt).toLocaleString() : assignment.createdAt ? new Date(assignment.createdAt).toLocaleString() : '—'}</div>
         </div>
       </div>
     </div>
   )}
